...
move<-function(x,action){
  ...
  if(sum(apply(abs(apply(wl,1,function(y){y-x})),2,sum) == 0) > 0){
    x<-x2
  }else{
    x<-x
  }
...
for(i in 1:50000){
  ...
  wl<-matrix(c(rep(10,5),c(1:5)),ncol=2,byrow=F)
  wl2<-matrix(c(c(6:10),rep(13,5)),ncol=2,byrow=F)
  wl<-rbind(wl,wl2)
    ...
    if(episode_done){
      ...
      if(bf_stage < stage_n){
        ...
        if(stage_n==2){
          ...
          condition<-F
          st<-c(1,1)
          wl<-matrix(c(rep(10,5),c(16:20)),ncol=2,byrow=F)
          wl2<-matrix(c(c(1:8),rep(5,8)),ncol=2,byrow=F)
          wl<-rbind(wl,wl2)
        ...
        if(stage_n==4){
          ...
          st<-c(1,20)
          goal_point<-c(20,1)
          wl<-matrix(c(rep(12,5),c(16:20)),ncol=2,byrow=F)
          wl2<-matrix(c(c(3:7),rep(15,5)),ncol=2,byrow=F)
          wl<-rbind(wl,wl2)
        }
...
plot_f<-function(obs2,stage_n,sts){
  ...
  fig <- plot_ly(obs,x=obs[,1], y= obs[,2],z=1,type = 'scatter3d', mode = 'lines',name="Path",
    ...
    add_trace(x=wl[,1], y= wl[,2], z=1,type = 'scatter3d',mode = 'lines',name="wall",
              line = list(width = 6, color = '#5e5e5f'),
              marker = list(size = 10, color = "#5e5e5f", cmin = -20, cmax = 50)) %>% 
    add_trace(x=wl2[,1], y= wl2[,2], z=1,type = 'scatter3d',mode = 'lines',name="wall",
              line = list(width = 6, color = '#5e5e5f'),
              marker = list(size = 10, color = "#5e5e5f", cmin = -20, cmax = 50)) %>% 
