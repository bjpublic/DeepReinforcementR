...
return_reward<-function(next_state,current_state){
  ...
  if(re_index==10000){
    reward<- 50 # episode end
    done<-T
  }
  else if(sum(c(261:270,341:350,361:370,441:450,461:470,511:520,531:540,611:620,631:640,701:720,731:740,861:890) %in% re_index) ==1){
    reward<- -2
    done<-F
  }else{
    reward <- -1
...
for(i in 1:10000){
  ...
  st2<-NULL
   ...
   while(episode_done==0){
    ...
    memory[[epoch_index]] <- list(input=convert_cur_state,action=next_action,reward=re_ep[1]+inr,done=re_ep[2],next_state=convert_next_state)
    episode_buffer[[bi]] <- list(input=convert_cur_state,action=next_action,reward=re_ep[1]+inr,done=re_ep[2],next_state=convert_next_state)
    bi<-bi+1
...
# rf<-colorRampPalette(c("#00007F", "blue", "#007FFF", "cyan",
#                        "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"))
# rf<-colorRampPalette(c("#FFFFD4", "#FED98E", "#FE9929", "#D95F0E", "#993404"))
png("sil_rnd.png",width=500,height = 500,res=100)
library(hexbin)
hexbinplot(X2~X1, data=zzz, colramp=rf, mincnt=1,trans=log, inv=exp,xlim=c(0, 100),ylim=c(0,100), aspect=1,colorkey = F)
dev.off()
...
total_inp<-t(apply(qut,1,convert_coord))
tn<-nn.ff(tn,total_inp)
a1<-tn$post[[length(size2)]]
pn<-nn.ff(pn,total_inp)
...
p = plot_ly(x = zzz3[,1],
            ...
            contours = list(
              ...
              size =(0.0235- min(loss))/5
            ))

print(p)


htmlwidgets::saveWidget(p, paste0("sil_rnd.html"))
