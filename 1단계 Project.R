...
convert_coord<-function(x,bonus_index,penalty_index,condition){
  wv2<-wv
  yv2<-yv
  wv2[x[1]]<-1
  yv2[x[2]]<-1
  ...
  c(wv2,yv2,absx,absy,bonusx,bonusy,abspx,abspy,penaltyx,penaltyy,condition)
...
return_reward<-function(st,condition){
...
bonus_index<-c(15,12)
penalty_index<-c(5,4)
init_data<-convert_coord(c(1,1),bonus_index,penalty_index,F)
goal_point<-c(20,20)
...
for(i in 1:10000){
    ...
    condition<-F
    st_2<-NULL
    init_data<-convert_coord(c(1,1),bonus_index,penalty_index,condition)
    while(episode_done==0){
      ...
      convert_cur_state<-convert_coord(st,bonus_index,penalty_index,condition)
      st<-move(st,next_action)
      st_2<-rbind(st_2,st)
      
      state_index<-stm[st[1],st[2]]    
      re_ep<-return_reward(st,condition)
      
      condition<-re_ep[3]
      convert_next_state<-convert_coord(st,bonus_index,penalty_index,condition)    
      ...
      if(episode_done | step %% epoch == 0){
        ...
        for(p in 1:nrow(yy)){
          yy[p,action %in%action_stack[p]] <- log(yy[p,action %in%action_stack[p]]+0.0001) 
        }
    ...
    if(i %% 20 ==0){
      ...
      for(t in 1:batch_num){      
        ...
        for(ml in 1:nrow(yy)){
          yy[ml,action %in% action_stack[ml]]<-
            log(yy[ml,action %in%action_stack[ml]]+0.0001)
        }
...
layout(autosize =F, aspectmode = 'manual', scene = list(xaxis=axx,yaxis=axy,zaxis=axz, aspectratio = list(x = 2, y = 2, z = 0.2))) %>% 
    add_trace(x=penalty_index[1], y= penalty_index[2], z=1,type = 'scatter3d',mode = 'lines+markers',name="Hole",
              line = list(width = 6, color = '#004e66'),marker = list(size = 10, color = "#004e66", cmin = -20, cmax = 50)) %>%
...
add_trace(x=1, y= 1, z=1,type = 'scatter3d',mode = 'lines+markers',name="Start",
          line = list(width = 6, color = '#ff5f2e'),
          marker = list(size = 10, color = "#ff5f2e", cmin = -20, cmax = 50)) %>%
add_trace(x=20, y= 20, z=1,type = 'scatter3d',mode = 'lines+markers',name="Goal",
...
add_trace(x=20, y= 20, z=1,type = "scatter3d", mode="text", text = "Goal", inherit=FALSE) %>% 
  add_trace(x=1, y= 1, z=1,type = "scatter3d", mode="text", text = "Start", inherit=FALSE) %>% 
