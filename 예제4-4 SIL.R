...
for(i in 1:10000){
  ...
  while(episode_done==0){
    ...
    episode_buffer[[bi]]<-  list(input=convert_cur_state,action=next_action,reward=re_ep[1],done=re_ep[2],next_state=convert_next_state,step=step)
    bi<-bi+1
    ...
    if(step == 500 |episode_done==1){
      ...
      ts.plot(reward_list,main=paste0("SIL"))
          break;
    }
...
write.csv(reward_list,"SIL_state_100_100.csv",row.names=F)

setwd("D:\\개인폴더\\책\\R로하는 강화학습\\코드")
# write.csv(reward_list,"A2C_state_100_100.csv",row.names=F)

base<-read.csv("A2C_state_100_100.csv")
...
for(j in 1:(length(reward_list)-10)){
  ...
  pl1[j]<-mean(reward_list[j:(j+20)])
  pl2[j]<-mean(base[j:(j+20),1])  
